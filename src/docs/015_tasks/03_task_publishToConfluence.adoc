...
If this option is present and equal to `swagger-open-api` then any source block marked with `class openapi` will be wrapped in the Open API (Swagger) Integration for Confluence (see https://marketplace.atlassian.com/apps/1218914/open-api-swagger-editor-for-confluence?hosting=cloud&tab=overview[Open API (Swagger) Editor for Confluence]).
